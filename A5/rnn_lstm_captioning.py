...
to_double_cuda = {'dtype': torch.float64, 'device': torch.device('cuda')}
...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h * (1 - torch.pow(torch.tanh(before_tanh), 2))
    dx = dtanh.mm(Wx.t())
...
def rnn_backward(dh, cache):
    ...
    dWh = torch.zeros_like(cache[0][2], **to_double_cuda)
    dWx = torch.zeros_like(cache[0][3], **to_double_cuda)
    dh0 = torch.zeros(N, H, **to_double_cuda)
    dx = torch.zeros(N, T, D, **to_double_cuda)

    for t in range(T)[::-1]:
        dout = dprev_h + dh[:, t]
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dout, cache[t])
        db += db_t
        dWh += dWh_t
        dWx += dWx_t
        dx[:, t] = dx_t
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_reshaped = x.reshape(-1, x.shape[-1]).to('cuda')
    y_reshaped = y.reshape(-1).to('cuda')
    loss = torch.nn.functional.cross_entropy(x_reshaped, y_reshaped, ignore_index=ignore_index, reduction='sum') / \
           x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # (N, T, H) => (N, T, V)
        self.Output_Projection = nn.Linear(hidden_dim, vocab_size, device=self.device, dtype=self.dtype)
    ...
    def forward(self, images, captions):
        ...
        H = H.to(self.Output_Projection.weight.dtype)
        # (4) (N,T,H) => (N, T, V)
    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length):
            ...
            next_h = next_h.to(self.Output_Projection.weight.dtype)
