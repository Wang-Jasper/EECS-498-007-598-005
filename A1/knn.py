...
def compute_distances_two_loops(x_train, x_test):
  ...
  train_image = x_train.view(num_train, -1)
  test_image  = x_test.view(num_test, -1)
  # making it a two dim
  for i in range(num_train):
      for j in range(num_test):
          dists[i][j] = (train_image[i] - test_image[j]).pow(2).sum()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  train_image = x_train.view(x_train.shape[0], -1)
  test_image = x_test.view(x_test.shape[0], -1)
  for i in range(num_train):
    dists[i] = ((train_image[i] - test_image) ** 2).sum(dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  train_image = x_train.view(num_train, -1)
  test_image = x_test.view(num_test, -1)
  train_square = train_image.pow(2).sum(-1, True)
  test_square = test_image.pow(2).sum(-1)
  xy = torch.matmul(train_image, test_image.T) # matrix multiplication, making the result has shape (num_train, num_test)
  dists = train_square + test_square - 2 * xy
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  # find the k smallest dists
  index = torch.topk(dists, k, dim=0, largest=False).indices
  # return the label that appear most frequently, if tie return the smallest
  for i in range(num_test):
    y_pred[i] = y_train[index[:, i]].bincount().argmax()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
class KnnClassifier:
  def __init__(self, x_train, y_train):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    self.train_image, self.train_label = x_train, y_train
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
  ...
  def predict(self, x_test, k=1):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dists = compute_distances_no_loops(self.train_image, x_test)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      train_image_list = x_train_folds[:i] + x_train_folds[i+1:]
      train_label_list = y_train_folds[:i] + y_train_folds[i+1:]
      test_image, test_label = x_train_folds[i], y_train_folds[i]

      train_image, train_label = torch.cat(train_image_list), torch.cat(train_label_list)

      classifier = KnnClassifier(train_image, train_label)
      accuracy = classifier.check_accuracy(test_image, test_label, k, True)
...
def knn_get_best_k(k_to_accuracies):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  f = lambda k: sum(k_to_accuracies[k])
  best_k = max(k_to_accuracies, key=f)
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
  return best_k
