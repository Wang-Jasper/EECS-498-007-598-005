...
def multiples_of_ten(start, stop):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  x = torch.zeros((stop - start) // 10, dtype=torch.float64)
  if x.shape != torch.Size([0]):
    x[0] = (start + 9) // 10 * 10
    for index in range(len(x[1:])):
      x[index + 1] = x[index] + 10
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def reverse_rows(x):
  ...
  idx1 = torch.arange(y.shape[0]).flip(dims=[0])
  y[idx0] = x[idx1]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
...
def normalize_columns(x):
  ...
  column_sigma = torch.sqrt(((x - column_means) ** 2).sum(dim=0) / (x.shape[0] - 1))

  y = (x - column_means) / column_sigma
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
