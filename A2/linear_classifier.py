...
def svm_loss_naive(W, X, y, reg):
  ...
  """
    ...
    # Replace "pass" statement with your code
    dW /= num_train
    dW += 2 * reg * W  # the derivative of the regularization term
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def sample_batch(X, y, num_train, batch_size):
  ...
  """
    ...
    #########################################################################
    # Replace "pass" statement with your code
    indixies = torch.randint(0, num_train, (batch_size,))
    X_batch = X[indixies]
    y_batch = y[indixies]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def train_linear_classifier(loss_func, W, X, y, learning_rate=1e-3,
  ...
  """
    ...
    num_train, dim = X.shape  # (N, D) N training examples and D dimension for each
    if W is None:
        # lazily initialize W
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  """
    ...
    # Replace "pass" statement with your code
    train_img = data_dict['X_train']
    val_img = data_dict['X_val']
    train_label = data_dict['y_train']
    val_label = data_dict['y_val']

    cls.train(train_img, train_label, lr, reg, num_iters)
    train_acc = (cls.predict(train_img) == train_label).sum().item() / len(train_label)
    val_acc = (cls.predict(val_img) == val_label).sum().item() / len(val_label)
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
