...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    out_h1, cache_h1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache_z = Linear.forward(out_h1, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dx, dw, db = Linear.backward(dout, cache_z)
    grads['W2'] = dw + 2 * self.reg * self.params['W2']
    grads['b2'] = db

    dx, dw, db = Linear_ReLU.backward(dx, cache_h1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    # Replace "pass" statement with your code
    layer_width = input_dim  # X * W, the width is the size of the input
    for index, hidden_dim in enumerate(hidden_dims):
      self.params['W{}'.format(index + 1)] = weight_scale * torch.randn(layer_width, hidden_dim, dtype=dtype, device=device)
      self.params['b{}'.format(index + 1)] = torch.zeros(hidden_dim, dtype=dtype, device=device)
      layer_width = hidden_dim

    # output layer
    self.params['W{}'.format(len(hidden_dims)+1)] = weight_scale * torch.randn(layer_width, num_classes, dtype=dtype, device=device)
    self.params['b{}'.format(len(hidden_dims)+1)] = torch.zeros(num_classes, dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for index in range(self.num_layers - 1):
      output, cache_dict['layer{}'.format(index + 1)] = Linear_ReLU.forward(output, self.params['W{}'.format(index + 1)], self.params['b{}'.format(index + 1)])
      if self.use_dropout:
        output, cache_dict['dropout_layer{}'.format(index + 1)] = Dropout.forward(output, self.dropout_param)

    scores, cache_dict['layer{}'.format(self.num_layers)] = Linear.forward(output, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    ############################################################################
    # Replace "pass" statement with your code
    loss, dout = softmax_loss(scores, y)  # dout is the gradient of scores
    loss += self.reg * (self.params['W{}'.format(self.num_layers)] ** 2).sum()

    previous_dout, dw, db = Linear.backward(dout, cache_dict['layer{}'.format(self.num_layers)])
    grads['W{}'.format(self.num_layers)] = dw + 2 * self.params['W{}'.format(self.num_layers)] * self.reg
    grads['b{}'.format(self.num_layers)] = db

    for index in range(self.num_layers - 1)[::-1]:
      if self.use_dropout:
        previous_dout = Dropout.backward(previous_dout, cache_dict['dropout_layer{}'.format(index + 1)])
      previous_dout, dw, db = Linear_ReLU.backward(previous_dout, cache_dict['layer{}'.format(index + 1)])
      grads['W{}'.format(index + 1)] = dw + 2 * self.params['W{}'.format(index + 1)] * self.reg
      grads['b{}'.format(index + 1)] = db

      loss += self.reg * ((self.params['W{}'.format(index + 1)] ** 2).sum())
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model, data_dict, num_epochs=100, device=device)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def rmsprop(w, dw, config=None):
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  config['cache'] = config.get('decay_rate') * config['cache'] + (1 - config.get('decay_rate')) * dw ** 2
