...
class Conv(object):
    ...
    Input:
    - x: Input data of shape (N, C, H, W), where N is the batch size, C is the number of channels
    - w: Filter weights of shape (F, C, HH, WW), where F is the number of filters
    - b: Biases, of shape (F,)
    ...
    """
        ...
        #  this is elementwise multiply so no transposing
        for n in range(N):
            for f in range(F):
                for hi in range(H_dout):
                    for wi in range(H_dout):
                        db[f] += dout[n, f, hi, wi]
...
class MaxPool(object):
        ...
        for n in range(N):
            for h in range(out_H):
                for w in range(out_W):
                    val, _ = x[n, :, h * stride: h * stride + pool_h, w * stride: w * stride + pool_w].reshape(C,
                                                                                                               -1).max(
                        dim=1)
                    out[n, :, h, w] = val
        #############################################################################
    ...
    """
        ...
        for n in range(N):
            for h in range(out_H):
                for w in range(out_W):
                    x_poWindow = x[n, :, h * stride: h * stride + pool_h,
                                 w * stride: w * stride + pool_w]  # the pooling window
                    reshaped_x_poWindow = x_poWindow.reshape(C, -1)  # to find the max in per channel
                    dx_tmp = torch.zeros_like(reshaped_x_poWindow)
                    val, index = reshaped_x_poWindow.max(-1)
                    dx_tmp[range(C), index] = dout[n, :, h, w]
                    dx[n, :, h * stride: h * stride + pool_h, w * stride: w * stride + pool_w] = dx_tmp.reshape(
                        x_poWindow.shape)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
...
class ThreeLayerConvNet(object):
  ...
  """
    ...
    """
        ...
        self.params['W{}'.format(1)] = weight_scale * torch.randn(num_filters, C, filter_size, filter_size, dtype=dtype,
                                                                  device=device)
        self.params['b{}'.format(1)] = torch.zeros(num_filters, dtype=dtype, device=device)

        self.params['W{}'.format(2)] = weight_scale * torch.randn(num_filters * H_out * W_out, hidden_dim, dtype=dtype,
                                                                  device=device)
    ...
    """
        ...
        scores, cache_L = Linear.forward(out_LR, self.params['W3'], self.params['b3'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class DeepConvNet(object):
  ...
  """
    ...
    """
        ...
        # {conv - [batchnorm?] - relu - [pool?]} x (L - 1) - linear
        C, H, W = input_dims
        channels = [C] + num_filters # the channels after each layer

        for index, num_filter in enumerate(num_filters):
            if weight_scale == 'kaiming':
                self.params[f'W{index + 1}'] = kaiming_initializer(channels[index], num_filter, K=3, relu=True, device=device, dtype=dtype)
            else:
                self.params[f'W{index + 1}'] = weight_scale * torch.randn(num_filter, channels[index], 3, 3, dtype=dtype, device=device)
            self.params[f'b{index + 1}'] = torch.zeros(num_filter, dtype=dtype, device=device)

            if self.batchnorm:
                self.params[f'gamma{index + 1}'] = torch.ones(num_filter, dtype=dtype, device=device)
                self.params[f'beta{index + 1}'] = torch.zeros(num_filter, dtype=dtype, device=device)
        ...
        if weight_scale == 'kaiming':
            ...
            self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(fc_dim, num_classes, dtype=dtype,
                                                                            device=device)
        ...
        for i in range(self.num_layers - 1):
            ...
            if i in self.max_pools:
                if self.batchnorm:
                    ...
                    out, cache = Conv_BatchNorm_ReLU_Pool.forward(out, w, b, gamma, beta, conv_param, bn_param,
                                                                  pool_param)
                    caches.append(cache)
                else:
